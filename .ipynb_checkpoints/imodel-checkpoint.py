...
class Model(iFile):
    ...
    _GAME=None;
    _API = None;
    _BUILD = None; #model architecture/layers
    _MAKE = 0; #Same build, but differnet make parameters
    _VERSION = None; #training / epoch 
    ...
    def load_latest_checkpoint(self):
        _MAX_CHKPNT = self.get_max_checkpoint();
        self._VERSION='e' + str(_MAX_CHKPNT).zfill(4);
        self._INFO['VERSION'] = self._VERSION;
        MaxFileName = self._GAME + '.' + self._API + '.' + self._BUILD + '.' + self._MAKE + '.' + self._VERSION + '.h5';
        self._FILE_NAME = MaxFileName;
        self.load_checkpoint();
        
    def load_checkpoint(self):
        chkpnt_file_path = self._CHECKPOINTS_FOLDER + '/' + self._FILE_NAME;
    ...
    def load_latest_version(self):
        print("[imodel: load_latest_version] Loading")
